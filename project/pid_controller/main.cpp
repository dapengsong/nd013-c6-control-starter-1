...
/*find_closest_index**/
int find_closest_index(double x_position,double y_position,vector<double> x_points,vector<double> y_points)
{
  int closest_index = 0;
...
int main() {
  ...
  pid_steer.Init(0.01,0.001,0.5,1.2,-1.2);
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer,
    ...
    if (s != "") {
      ...
      //error_steer = 0;
      // x_points, y_points
      // error_steer is calculated by desired yaw of the  point(the closest point to the current position) - the actual yaw
      double dis_min = DBL_MAX;
      int closest_index = find_closest_index(x_position,y_position,x_points,y_points);

      error_steer = angle_between_points(x_position,y_position,x_points[closest_index], y_points[closest_index])-yaw;
      ...
      // error_throttle is calculated by the arctan of desired speed of the point(the closest point to the current position) and the actual speed
